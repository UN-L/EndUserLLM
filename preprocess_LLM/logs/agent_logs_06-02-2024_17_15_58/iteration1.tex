...
% Import the standalone class for creating images that can be compiled on their own
\documentclass[tikz,border=5]{standalone}

% Import the xcolor-material package with a custom prefix to use Material Design colors
...
% Define various TikZ styles and commands for creating the dog illustration
\tikzset{%
  ...
  execute/.code={#1},
  % Define styles for applying styles to the left and right halves of a symmetrical shape
...
% Define the dog illustration using TikZ commands
\tikzset{%
dog/.pic={
\begin{scope}[x=3cm/480,y=3cm/480]
\useasboundingbox (-256, -256) (256, 256);
% Create the main head shape with blue colors
\reflect[split=Blue400 and Blue500]{
...
\begin{document}
\begin{tikzpicture}
% Loop through each animal (in this case, only 'dog') and position them on the canvas
\foreach \animal [count=\i from 0, evaluate={\x=mod(\i,3)*5; \y=-floor(\i/3)*5;}]
  in {dog}{
\pgfmathrandomitem\materialColor{material}
% Position the dog and draw a background circle with a random light color
\scoped[shift={(\x,\y)}]{
\fill [fill=\materialColor50] circle [radius=2];
% Draw the dog illustration
\pic {\animal};
