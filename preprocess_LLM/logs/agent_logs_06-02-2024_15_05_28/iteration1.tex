...
\reflect[split=Brown400 and Brown500]{ % Create the brown patches on the dog's face
  \fill [fill color] (0,-64) ellipse [x radius=160, y radius=144]; % The main head shape
  \fill [fill color] (0, 32) ellipse [x radius=128, y radius=112]; % The upper head shape
  \fill [fill color] (32,96) % The shape for the ears and top of the head
  .. controls ++( 75:128) and ++(105:128) .. ++(192,  0)
  .. controls ++(285: 96) and ++(285: 96) .. ++(-80,-32)
  .. controls ++(105: 64) and ++( 75: 32) .. cycle;
}
\reflect[split={Grey100 and Grey200}]{ % Create the white part of the face (snout)
  \clip (0,-64) ellipse [x radius=160, y radius=144]; % Clip to the head shape
  \fill [fill color](0,-224) % The snout and lower head shape
...
\reflect[split={Pink100 and Pink200}]{ % Create the pink tongue
  \fill [fill color](0,-192) ellipse [x radius=28, y radius=32]; % The tongue shape
}
\reflect[split={BlueGrey800 and BlueGrey900}]{ % Create the nose and mouth
  \fill [fill color](0,-144) % The mouth shape
    .. controls ++(  0:20) and ++(315:20) .. ++( 40,64)
    .. controls ++(135:20) and ++( 45:20) .. ++(-80, 0)
    .. controls ++(225:20) and ++(180:20) .. cycle;
  \fill [BlueGrey900] (56, 0) circle [radius=20]; % The nose shape
  \fill [fill color] (-8,-112) % The smiling mouth line
  -- ++(16,0) -- ++(0,-32) arc (180:360:24)
  arc (180:0:8) arc (360:180:40);
  \fill [fill=blue] (-40, -96) circle [radius=8]; % The left eye shape
  \fill [fill=blue] (40, -96) circle [radius=8]; % The right eye shape
}
\end{scope}}
}
...
\foreach \animal [count=\i from 0, evaluate={\x=mod(\i,3)*5; \y=-floor(\i/3)*5;}]
  in {dog}{ % Loop to create multiple instances of the dog picture
\pgfmathrandomitem\materialColor{material} % Randomly select a background color
\scoped[shift={(\x,\y)}]{ % Position each dog picture
