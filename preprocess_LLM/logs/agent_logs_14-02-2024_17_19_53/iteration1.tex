...
% Import the standalone class with TikZ and set a border of 5
...
% Define TikZ styles for various effects
\tikzset{%
  ...
  execute/.code={#1},
  % Define styles for applying effects to the left or right side
  on left/.style={.. on left/.style={#1}},
  on right/.style={.. on right/.style={#1}},
  % Define a style for splitting the color effect between left and right
...
% Define a new command for creating a reflection effect
\newcommand\reflect[2][]{%
...
% Define the dog pic with the stylized illustration
\tikzset{%
dog/.pic={
\begin{scope}[x=3cm/480,y=3cm/480]
\useasboundingbox (-256, -256) (256, 256);
% Create the split color effect for the dog's face
\reflect[split=Brown400 and Brown500]{
  % Draw the main face shape with the brown color
  \fill [fill color] (0,-64) ellipse [x radius=160, y radius=144];
  % Draw the top part of the head with the brown color
  \fill [fill color] (0, 32) ellipse [x radius=128, y radius=112];
  % Draw the ears and the top of the head with the brown color
  \fill [fill color] (32,96)
  .. controls ++( 75:128) and ++(105:128) .. ++(192,  0)
  .. controls ++(285: 96) and ++(285: 96) .. ++(-80,-32)
  .. controls ++(105: 64) and ++( 75: 32) .. cycle;
}
% Create the split color effect for the white area of the face
\reflect[split={Grey100 and Grey200}]{
  % Clip the drawing to the main face shape
  \clip (0,-64) ellipse [x radius=160, y radius=144];
  % Draw the white area running down the center of the face
...
% Create the split color effect for the tongue
\reflect[split={Pink100 and Pink200}]{
  % Draw the tongue with the pink color
  \fill [fill color](0,-192) ellipse [x radius=28, y radius=32];
}
% Create the split color effect for the nose and the collar
\reflect[split={BlueGrey800 and BlueGrey900}]{
  % Draw the nose with the blue-grey color
  \fill [fill color](0,-144) 
    .. controls ++(  0:20) and ++(315:20) .. ++( 40,64)
    .. controls ++(135:20) and ++( 45:20) .. ++(-80, 0)
    .. controls ++(225:20) and ++(180:20) .. cycle;
  % Draw the collar with a darker blue-grey color
  \fill [BlueGrey900] (56, 0) circle [radius=20];
  % Draw the mouth and the tongue sticking out
  \fill [fill color] (-8,-112)
  -- ++(16,0) -- ++(0,-32) arc (180:360:24)
  arc (180:0:8) arc (360:180:40);
}
% Change the color of the left eye to red
\reflect[split={Red400 and Brown500}]{
  % Draw the left eye with the red color
  \fill [fill color] (-64, 0) ellipse [x radius=32, y radius=40];
}
\end{scope}}
}
...
\begin{document}
\begin{tikzpicture}
% Loop through the animals (in this case, only 'dog') and position them
...
\pic {\animal};
% Add a label below the illustration with the animal's name
