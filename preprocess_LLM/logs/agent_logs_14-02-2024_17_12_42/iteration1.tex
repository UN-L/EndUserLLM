...
% Define TikZ settings for various styles used in the illustration
\tikzset{%
  % Define a half clip to be used for creating split color effects
  half clip/.code={
    \clip (0, -256) rectangle (256, 256);
  },
  % Define a color setting to use named colors
  color/.code=\colorlet{fill color}{#1},
  % Define a color alias for reusing colors with different names
  color alias/.code args={#1 as #2}{\colorlet{#1}{#2}},
  % Define a style for applying multiple color aliases
  colors alias/.style={color alias/.list/.expanded={#1}},
  % Define a style for executing code
  execute/.code={#1},
  % Define styles for applying settings to the left or right side of a split
  on left/.style={.. on left/.style={#1}},
  on right/.style={.. on right/.style={#1}},
  % Define a split style for applying different colors to each side
...
% Create the head and ears with a split brown color effect
...
% Create the muzzle with a split grey color effect
...
% Create the tongue with a split pink color effect
\reflect[split={Pink100 and Pink200}]{
  \fill [fill color](0,-192) ellipse [x radius=28, y radius=32];
}
% Create the nose and mouth with a split blue-grey color effect
...
\reflect[split={Red400 and BlueGrey900}]{
  \fill [fill color] (-48, 32) ellipse [x radius=24, y radius=16];
}
\end{scope}}
}
