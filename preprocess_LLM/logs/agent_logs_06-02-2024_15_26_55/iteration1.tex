...
% Define the document class with TikZ and a border specification
...
% Define TikZ styles and commands for creating the dog image
\tikzset{%
  % Define a half clipping path to be used for symmetry
  half clip/.code={
  ...
  execute/.code={#1},
  % Define styles for left and right sides of the symmetry
  on left/.style={.. on left/.style={#1}},
  on right/.style={.. on right/.style={#1}},
  % Define a style for splitting the left and right colors
...
% Define a command to reflect shapes for creating symmetry
\newcommand\reflect[2][]{%
...
% Define the dog image using TikZ commands
\tikzset{%
dog/.pic={
\begin{scope}[x=3cm/480,y=3cm/480]
\useasboundingbox (-256, -256) (256, 256);
% Create the dog's ears and top of the head with brown colors
...
% Create the white face with a brown patch
...
% Create the dog's eyes and tongue with dark colors
\reflect[split={Red800 and Red900}]{
  \fill [fill color](0,-144) 
    .. controls ++(  0:20) and ++(315:20) .. ++( 40,64)
    .. controls ++(135:20) and ++( 45:20) .. ++(-80, 0)
    .. controls ++(225:20) and ++(180:20) .. cycle;
  \fill [Red900] (56, 0) circle [radius=20];
...
% Position the dog image and create the background circle
\scoped[shift={(\x,\y)}]{
\fill [fill=\materialColor50] circle [radius=2];
\pic {\animal};
% Add the label "dog" below the image
