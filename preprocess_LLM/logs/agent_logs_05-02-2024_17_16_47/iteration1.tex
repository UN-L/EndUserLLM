...
\reflect[split=Brown400 and Brown500]{
  \fill [fill color] (0,-64) ellipse [x radius=160, y radius=144];  % Fills the bottom part of the face with a brown color
  \fill [fill color] (0, 32) ellipse [x radius=128, y radius=112];  % Fills the top part of the face with a brown color
  \fill [fill color] (32,96)
  .. controls ++( 75:128) and ++(105:128) .. ++(192,  0)
  .. controls ++(285: 96) and ++(285: 96) .. ++(-80,-32)
  .. controls ++(105: 64) and ++( 75: 32) .. cycle;  % Fills the snout area with a brown color
}
\reflect[split={Grey100 and Grey200}]{
  \clip (0,-64) ellipse [x radius=160, y radius=144];  % Clips the bottom part of the face
  \fill [fill color](0,-224) 
    .. controls ++(  0:64) and ++(270:64) .. ++(96,64)
    .. controls ++( 90:64) and ++(270:64) .. ++(-96,192)
    .. controls ++(270:64) and ++( 90:64) .. ++(-96,-192)
    .. controls ++(270:64) and ++(180:64) .. cycle;  % Fills the white patch on the face
}
\reflect[split={Pink100 and Pink200}]{
  \fill [fill color](0,-192) ellipse [x radius=28, y radius=32];  % Fills the pink tongue sticking out
}
\reflect[split={BlueGrey800 and BlueGrey900}]{
  \fill [fill color](0,-144) 
    ...
    .. controls ++(225:20) and ++(180:20) .. cycle;  % Fills the black nose
  \fill [Red] (56, 0) circle [radius=20];  % Fills the eyes with red color
  \fill [fill color] (-8,-112)
  -- ++(16,0) -- ++(0,-32) arc (180:360:24)
  arc (180:0:8) arc (360:180:40);  % Fills the ears with a grey color
}
\end{scope}}
}
\begin{document}
...
\foreach \animal [count=\i from 0, evaluate={\x=mod(\i,3)*5; \y=-floor(\i/3)*5;}]
  in {dog}{
\pgfmathrandomitem\materialColor{material}  % Randomly selects a color from the material color list
\scoped[shift={(\x,\y)}]{
\fill [fill=\materialColor50] circle [radius=2];  % Fills the background with a muted green color
